...
- 默认收起的 Task Side Panel，复刻官方 New tab 启动页：Review 打开真实 Diff，Files 连接受限项目选择器；仅原生桥接支持的 Terminal、Browser 和 Side task 会明确提示当前不可用，不会伪造工具界面
...
界面按 Codex Desktop 结构拆分为独立的 Bottom Panel、Summary 和 Task Side Panel。当前 app-server 桥接提供真实 Review 流程和受限的本地项目选择；仅原生 Desktop 支持的 Terminal、内嵌 Browser 和 Side task 入口仍保留结构一致性，但会明确提示不可用，不会伪造对应功能。
