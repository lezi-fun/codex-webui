...
- 带目录边界检查的项目文件夹选择器
- Codex 风格的侧栏账号身份，支持姓名、头像 fallback、邮箱和套餐
- 长会话窗口化，避免一次渲染数千条 Turn
- 桌面与移动端响应式布局，手机端使用全屏 Review 抽屉
- 默认监听 `0.0.0.0:8899`，可供局域网设备访问
...
服务启动时还会打印检测到的局域网地址，例如：

```text
LAN: http://192.168.x.x:8899
```

点击 Composer 或侧栏中的文件夹按钮，即可选择 Codex 的工作目录。
...
- 不要把 `8899` 端口直接暴露到公网。
- 仅在可信局域网、VPN 或带认证的反向代理后使用。
- 允许命令前认真阅读审批内容。
- 文件夹浏览被限制在当前用户 Home 目录中。
- Review Undo/Reapply 只接受配置的精确 Git 根目录。
- 账号身份由 app-server 的 `account/read` 与 Codex 已登录的 profile endpoint 聚合；`auth.json` 只在服务端读取，access/refresh token 不会返回浏览器。
