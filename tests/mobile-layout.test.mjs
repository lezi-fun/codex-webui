...
const terminal=await narrow.evaluate(()=>({title:document.querySelector('#bottomPanelTab').textContent.trim(),utilityVisible:!document.querySelector('#bottomPanelUtility').hidden}));
...
await browser.close();
if(result.viewport.scrollWidth>591||result.main.x<0||result.main.width<560||result.closedSummary.display!=='none'||result.closedSidePanel.display!=='none'||result.openSummary.width!==591||result.openSidePanel.width!==591||result.closedSidebar.x>=0||Math.abs(result.openSidebar.x)>0.5||result.openSidebar.width!==275||result.accountButton.height!==30||result.accountMenu.display==='none'||bottom.scrollWidth>320||bottom.panelWidth!==320||bottom.title!=='New tab'||bottom.utilityVisible||bottomLauncher.title!=='New tab'||!bottomLauncher.visible||!bottomLauncher.buttonsInside||terminal.title!=='Terminal'||!terminal.utilityVisible||summaryFocus.activeId!=='closeSummaryPanel'||!summaryFocus.conversationInert||summaryRestore.activeId!=='toggleSummaryPanel'||summaryRestore.conversationInert||sideFocus.activeId!=='closeSidePanel'||!sideFocus.conversationInert||!dialogEscape.sideOpen||sideRestore.activeId!=='toggleSidePanel'||sideRestore.conversationInert)process.exit(1);
